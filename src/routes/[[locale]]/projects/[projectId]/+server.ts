...
const map = (
    ...
    const minimum = Object.values(project.budget?.minimum ?? {}).reduce(
        (acc, { amount }) => acc + amount,
        0,
    );
    const optimum = Object.values(project.budget?.optimum ?? {}).reduce(
        (acc, { amount }) => acc + amount,
        0,
    );
...
const RewardsSample = [
    ...
    },
    {
        ...
        description:
            "Participe de uma sessão de perguntas e respostas exclusiva com a equipe do projeto. Vagas limitadas.",
        money: {
            amount: 100,
            currency: "EUR",
        },
