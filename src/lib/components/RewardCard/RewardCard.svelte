...
<Dialog.Root bind:open>
    <Dialog.Trigger class="text-left">
        <Card.Root class="w-full max-w-md drop-shadow-sm">
            <Card.Header>
                {#if size === "lg"}
                    <img src={image} alt="Reward" class="h-40 w-full object-cover" />
                {/if}
                ...
                {#if size === "lg"}
                    <div class="flex justify-between gap-4">
                        <p class="flex items-center gap-2 text-sm">
                            <img alt="donors" src={userIcon} class="h-6 w-6" />
                            {$number(donors)}
                            {$_("reward.donors")}
                        </p>
                        ...
                        {#if units !== null}
                            <p class="flex items-center gap-2 text-sm">
                                <img alt="donors" src={boxIcon} class="h-6 w-6" />
                                {#if units > 0}
                                    {$_("reward.units", {
                                        values: {
                                            units: $number(units, {
    ...
    <Dialog.Content class="w-full max-w-2xl gap-8">
        <Dialog.Header class="gap-2">
            <img src={image} alt="Reward" class="h-80 w-full object-cover" />
        </Dialog.Header>
