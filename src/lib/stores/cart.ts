...
const initialCart: CartStore = browser ? JSON.parse(localStorage.getItem("cart") || '{"items":[]}') : { items: [] };
...
function createCartStore() {
  ...
  return {
    ...
    addItem: (item: CartItem) =>
      update((cart) => {
        const existingItemIndex = cart.items.findIndex((i) => i.id === item.id && i.type === item.type);
...
export const itemCount = derived(cart, ($cart) => $cart.items.reduce((total, item) => total + item.quantity, 0));
