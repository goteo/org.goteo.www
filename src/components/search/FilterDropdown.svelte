...
<div class="relative w-full">
    {#if label}
        <label for={dropdownId} class="mb-2 block text-sm font-medium text-secondary">
            {label}
        </label>
    {/if}
    ...
    <button
        ...
        class="flex w-full items-center justify-between rounded-lg border border-secondary bg-white px-4 py-4 font-body text-base text-black focus:border-primary focus:ring-2 focus:ring-primary focus:outline-none {className}"
    ...
    {#if isOpen}
        <div
            class="absolute top-full right-0 left-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-lg border border-secondary bg-white shadow-lg"
        ...
        >
            <div class="py-1" role="listbox">
                {#each options as option}
                    <button
                        type="button"
                        class="hover:bg-variant1 focus:bg-variant1 w-full px-4 py-3 text-left font-body text-black focus:outline-none"
