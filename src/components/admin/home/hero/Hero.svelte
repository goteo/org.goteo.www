...
<!--
    Two columns: content on the left, Save on the right. The button's grid area spans every
    row, so it sits level with the heading and its sticky range covers the whole form.
-->
<form
    bind:this={formElement}
    onsubmit={handleSubmit}
    class="grid grid-cols-[minmax(0,1fr)_auto] gap-x-6 gap-y-10"
>
    <div class="col-start-1 row-start-1 flex max-w-167 flex-col gap-4">
    ...
    <ActionableButton
        action={submit}
        autoreset={2000}
        class="sticky top-(--sticky-top) z-20 col-start-2 row-start-1 row-end-3 w-fit self-start px-6"
    >
        {$t("common.save")}
    </ActionableButton>

    <div class="col-start-1 row-start-2 flex flex-col gap-10">
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex max-w-167 gap-6">
                <DateInput
                    bind:value={startsAt}
                    class="flex-1"
                    name="startsAt"
                    placeholder={$t("pages.admin.home.hero.fields.startDatePlaceholder")}
                    error={fieldErrors.startsAt && $t(fieldErrors.startsAt)}
                />
                ...
                <DateInput
                    ...
                    error={fieldErrors.endsAt && $t(fieldErrors.endsAt)}
                />
            </div>
        </div>
    </div>
