<script lang="ts">
    ...
    let title = $state(reward?.title ?? collab?.title ?? "");
    let description = $state(reward?.description ?? collab?.description ?? "");
    ...
    $effect(() => {
        if (!collab && reward) selectedStep = "rewards";
        else if (!reward && collab) selectedStep = "collabs";
    });
</script>
...
>
    ...
    <div class="flex flex-col gap-4">
        ...
        {:else if collab}
            <input
                type="text"
                placeholder={$t("wizard.steps.collabs.modal.placeholders.title")}
                class={inputsClass}
                value={collab === null ? "" : collab.title}
            />
            <textarea
                placeholder={$t("wizard.steps.collabs.modal.placeholders.description")}
                class={`h-32 resize-none ${inputsClass}`}
                >{collab === null ? "" : collab.description}</textarea
            >
        {/if}
    </div>

    {#snippet footer()}
        {#if (reward !== null && !collab) || (collab !== null && !reward)}
            <Button kind="secondary" onclick={() => handleDeleteClick()} class="w-fit">
                {$t(`wizard.steps.${selectedStep}.modal.btns.delete`)}
            </Button>
        {/if}
        <Button onclick={() => handleContinue()} class="w-fit">
            {$t(`wizard.steps.${selectedStep}.modal.btns.continue`)}
        </Button>
    {/snippet}
</Modal>
