            ...
            if ($body.find(".flex-col.gap-6 > .flex-row > .flex > .inline-block").length > 0) {
                ...
                cy.get("body").then(($body) => {
                    if ($body.find("form#payment").length > 0) {
                        cy.get("form#payment", { timeout: 3000 }).should("exist");

                        if ($body.find("label[data-gateway='stripe']").length > 0) {
                            cy.get("label[data-gateway='stripe']").should("be.visible").click();
                            cy.get("input[name='paymentMethod'][value='stripe']").should(
                                "be.checked",
                            );
                            ...
                            cy.log(
                                "Opción Stripe no encontrada, verificando formulario de pago general",
                            );
